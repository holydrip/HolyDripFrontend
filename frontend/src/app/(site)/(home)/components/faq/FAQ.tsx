...
export default function FAQ() {
    ...
    const label = data?.sectionLabel || t("faq_label");
    const heading = data?.title || t("faq_title");
    const footer = data?.footerText || t("faq_footer");
    ...
    return (
        <motion.section variants={sv} initial="hidden" whileInView="visible" viewport={{ once: true }}
            ...
            <div className="flex flex-col gap-2">
              ...
              <h2 className={`font-fraktur-en text-white`} style={{ fontSize: "clamp(32px, 5vw, 56px)" }}>
                {heading}
              </h2>
            </div>
            ...
            <p className={`font-fraktur-en text-white/15 text-2xl`}>
                {footer}
            </p>
        </motion.section>
    )
